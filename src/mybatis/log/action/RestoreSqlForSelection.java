...
public class RestoreSqlForSelection extends AnAction {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        if (sqlText.contains(StringConst.CREATEDS)) {
            sqlText = sqlText.replace(StringConst.PARAMETERS2, StringConst.PARAMETERS).replace(StringConst.CREATEDS, StringConst.PREPARING).replaceAll("\\n\\s+"," ");
        }
        generateSqlOld(project, configVo, sqlText);
    }


    private void generateSql(Project project, ConfigVo configVo, String sqlText) {

        System.out.println(sqlText);
        if (StringUtils.isNotBlank(sqlText) && sqlText.contains(StringConst.PARAMETERS2) && sqlText.contains(StringConst.CREATEDS)) {
            ...
            if (sqlArr != null && sqlArr.length >= 2) {
                for (int i = 0; i < sqlArr.length; ++i) {
                    ...
                    if (currentLine.contains(StringConst.PARAMETERS2) && StringUtils.isNotEmpty(prevLine) && prevLine.contains(StringConst.CREATEDS)) {
                    ...
                    } else if (currentLine.contains(StringConst.CREATEDS)) {
    ...
    private void generateSqlOld(Project project, ConfigVo configVo, String sqlText) {
